...
int Database::lockUsersSecurity()
{
	QSqlQuery sql(db);
	sql.exec("UPDATE users a INNER JOIN point b ON a.ID = b.uid SET passwd2 = 'locked' WHERE a.qq != '0' AND DATE_ADD(b.lastlogin, interval 1 hour) < now()");
	return sql.numRowsAffected();
}
